...
TextureShared TextureManager::loadTexture(const string& filename) {
    ...
    // Convert to a shared texture that cleans up with DestroyTexture
    TextureShared tex(
...
TextureManagerTwo::TextureManagerTwo(SDL_Renderer* renderer)
: renderer(renderer)
{ }

TextureShared TextureManagerTwo::loadObject(const std::string& filename) {
    // Grab resources
    if(filename.size()<=4)
        throw ResourceException("TextureManager", "Invalid filename, should be _.bmp or _.png");
